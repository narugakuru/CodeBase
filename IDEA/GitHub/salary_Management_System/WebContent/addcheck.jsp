...
<body>
	<%
		...
		if(!(user.equals(session.getAttribute("username")))){
			out.println("<script>alert('员工姓名不符，添加信息失败！');window.location.href='personinfo.jsp';</script>");
		}else{
			Statement createtable = connection.createStatement();
			int rscreate = createtable.executeUpdate(sql);
			if((user.equals(session.getAttribute("username"))) && rscreate>0){
				out.println("<script>alert('员工添加信息成功！');window.location.href='personinfo.jsp';</script>");			
			}else{
				out.println("<script>alert('员工添加信息失败！');window.location.href='personinfo.jsp';</script>");
			}			
		}
	%>
</body>
